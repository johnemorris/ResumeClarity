...
const Tooltip: React.FC<TooltipProps> = ({ content, children, position = 'top', width = 'w-64' }) => {
  ...
  return (
    ...
    >
      {children}
      {isVisible && (
        <div className={`absolute z-[100] ${positionClasses[position]} ${width} animate-in fade-in zoom-in-95 duration-200`}>
          <div className="bg-slate-950 text-white text-[11px] font-medium leading-relaxed p-4 rounded-2xl shadow-2xl border border-white/10 text-center">
            {content}
